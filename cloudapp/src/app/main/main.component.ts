    ...
    this.itemService
      ...
      .subscribe(
        (item) => {
          ...
          if (
            this.locationCheck &&
            item.item_data.library.value != this.library
          ) {
            this.playBeep("C3");
            this.alert.error(
              `Item with the barcode ${itemBarcode} is not in the ${this.library} library.`,
            );
            return;
          }
          ...
          if (!this.uniqueItemIds.has(uniqueId)) {
            ...
            this.playBeep("C3");
            this.alert.error(
              `Item with the barcode ${itemBarcode} already exists in the list.`,
            );
          }
        },
        (error) => {
  ...
  onSubmit() {
    ...
    forkJoin(updateRequests).subscribe({
      next: (response) => {
        if (this.bigMoveMode) {
          this.itemService
            .scanInItem(response[0], this.library, this.circDesk)
            .subscribe({
              next: (response) => {
              ...
              error: (error) => {
                console.error(error);
                this.alert.error(
                  "An error occurred while running scan-in: " + error.message,
                );
              },
            });
        }
      ...
      error: (error) => {
        ...
        console.error(error);
        this.alert.error(
          "An error occurred while updating the items: " + error.message,
        );
      },
    });
  }
